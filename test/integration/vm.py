...
class VM(abc.ABC):
    ...
    # XXX: classmethod?
    def _connect(self, user, password="", keyfile=None):
        if not self.running():
            self.start()
        client = SSHClient()
    ...
    def run(self, cmd, user, password="", keyfile=None):
        ...
        """
        client = self._connect(user, password, keyfile)
    ...
    # XXX: sync with bib vm.py
    def copy_to(self, user, host_path, remote_path, password="", keyfile=None):
        client = self._connect(user, password, keyfile)
        sftp = client.open_sftp()
        sftp.put(host_path, remote_path)
        sftp.close()

    def copy_from(self, user, remote_path, host_path, password="", keyfile=None):
        client = self._connect(user, password, keyfile)
        sftp = client.open_sftp()
        sftp.get(remote_path, host_path)
        sftp.close()
...
class QEMU(VM):
    ...
    def _gen_qemu_cmdline(self, snapshot, use_ovmf):
        ...
        elif self._arch == "riscv64":
            qemu_cmdline = [
                "qemu-system-riscv64",
                "-smp", "2",
                "-M", "virt,acpi=off",
                # XXX: this is /usr/share/edk2/riscv/ on fedora
