    ...
    package_sets:
      os:
        - include: [inc1]
          ...
          conditions:
            "some-description-1":
              when: distro_name == "test-distro"
              append:
                include: [from-condition-inc2]
                exclude: [from-condition-exc2]
            "some-description-2":
              when: distro_name == "other-distro"
              append:
                include: [inc3]
                exclude: [exc3]
      container:
  ...
  test_type:
    ...
    partition_tables_override:
      conditions:
            ...
            test_arch:
              ...
              partitions:
                - <<: *default_part_0
                  size: 111_111_111
        "some description-1":
          when: version_greater_or_equal("1") and version_less_than("2")
          override:
            test_arch:
              <<: *test_arch_pt
              partitions:
        ...
        "some description-2":
          when: version_greater_or_equal("2")
          override:
            test_arch:
              <<: *test_arch_pt
              partitions:
