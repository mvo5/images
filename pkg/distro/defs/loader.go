...
func (i *imageTypesCache) cacheKey(distroNameVer string, h hash.Hash) string {
	// we distroNameVer+hash as the various helpers mutate the
	// imageTypesYAML based on distro name/version conditionals
	return fmt.Sprintf("%s:%x", distroNameVer, h.Sum(nil))
}

func (i *imageTypesCache) Get(distroNameVer string, h hash.Hash) *imageTypesYAML {
	i.mu.Lock()
	defer i.mu.Unlock()

	return i.cache[i.cacheKey(distroNameVer, h)]
}

func (i *imageTypesCache) Set(distroNameVer string, h hash.Hash, ity *imageTypesYAML) {
	i.mu.Lock()
	defer i.mu.Unlock()

	i.cache[i.cacheKey(distroNameVer, h)] = ity
}
...
func load(distroNameVer string) (*imageTypesYAML, error) {
	...
	if cached := itCache.Get(distroNameVer, h); cached != nil {
		return cached, nil
	}
	...
	itCache.Set(distroNameVer, h, &toplevel)
