...
func TestLoadConditionDistro(t *testing.T) {
	...
	fakePkgsSetYaml := `
package_sets: 
  test_type_pkgset: &test_type_pkgset
    include: [inc1]
    exclude: [exc1]
    condition:
      distro_name:
...
image_types:
  test_type:
    package_sets:
      - *base_pkgset
      - *test_type_pkgset
      - include: [from-extra-inc]
        exclude: [from-extra-exc]
`
	// XXX: we cannot use distro.Name() as it will give us a name+ver
	baseDir := makeFakePkgsSet(t, test_distro.TestDistroNameBase, fakePkgsSetYaml)
	restore := packagesets.MockDataFS(baseDir)
	...
	assert.NotNil(t, pkgSet)
	assert.Equal(t, rpmmd.PackageSet{
		Include: []string{"from-base-condition-inc", "from-base-inc", "from-condition-inc", "from-extra-inc", "from-type-inc"},
		Exclude: []string{"from-base-condition-exc", "from-base-exc", "from-condition-exc", "from-extra-exc", "from-type-exc"},
	}, pkgSet)
}
