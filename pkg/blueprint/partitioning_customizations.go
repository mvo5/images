...
func (p *PlainFilesystemCustomization) validate() error {
	if p == nil {
		return nil
	}
...
func (p *LVMCustomization) validate(plainOnlyMountpoints []string) error {
	if p == nil {
		return nil
	}

	vgnames := make(map[string]bool)
	for _, vg := range p.VolumeGroups { // there can be only one VG currently, but keep the check for when we change the rule
...
func (p *BtrfsCustomization) validate(plainOnlyMountpoints []string) error {
	if p == nil {
		return nil
	}
...
func (p *PartitioningCustomization) Validate() error {
	...
	if err := p.LVM.validate(plainOnlyMountpoints); err != nil {
		return err
	}

	if err := p.Btrfs.validate(plainOnlyMountpoints); err != nil {
		return err
	}
