...
func TestDynamicallyResizePartitionTable(t *testing.T) {
	...
	newpt, err := disk.NewPartitionTable(&pt, mountpoints, 1024, disk.RawPartitioningMode, arches.AARCH64, nil, rng)
...
func TestCreatePartitionTable(t *testing.T) {
	...
	for ptName, pt := range testdisk.TestPartitionTables() {
		for bpName, bp := range testBlueprints {
			...
			mpt, err := disk.NewPartitionTable(&pt, bp, uint64(13*MiB), ptMode, arches.PPC64LE, nil, rng)
	...
	for bpName, tbp := range testBlueprints {
		for ptName, pt := range testdisk.TestPartitionTables() {
			if tbp != nil && (ptName == "btrfs" || ptName == "luks") {
				_, err := disk.NewPartitionTable(&pt, tbp, uint64(13*MiB), disk.AutoLVMPartitioningMode, arches.X86_64, nil, rng)
				assert.Error(err, "PT %q BP %q: should return an error with LVMPartitioningMode", ptName, bpName)
				continue
			}

			mpt, err := disk.NewPartitionTable(&pt, tbp, uint64(13*MiB), disk.AutoLVMPartitioningMode, arches.X86_64, nil, rng)
	...
	for bpName, tbp := range testBlueprints {
		for ptName, pt := range testdisk.TestPartitionTables() {
			if ptName == "auto-lvm" || ptName == "luks" || ptName == "luks+lvm" {
				_, err := disk.NewPartitionTable(&pt, tbp, uint64(13*MiB), disk.BtrfsPartitioningMode, arches.X86_64, nil, rng)
				assert.Error(err, "PT %q BP %q: should return an error with BtrfsPartitioningMode", ptName, bpName)
				continue
			}

			mpt, err := disk.NewPartitionTable(&pt, tbp, uint64(13*MiB), disk.BtrfsPartitioningMode, arches.X86_64, nil, rng)
			assert.NoError(err, "PT %q BP %q: Partition table generation failed: (%s)", ptName, bpName, err)
	...
	for bpName, tbp := range testBlueprints {
		for ptName, pt := range testdisk.TestPartitionTables() {
			if ptName == "btrfs" || ptName == "luks" {
				_, err := disk.NewPartitionTable(&pt, tbp, uint64(13*MiB), disk.LVMPartitioningMode, arches.S390X, nil, rng)
				assert.Error(err, "PT %q BP %q: should return an error with LVMPartitioningMode", ptName, bpName)
				continue
			}

			mpt, err := disk.NewPartitionTable(&pt, tbp, uint64(13*MiB), disk.LVMPartitioningMode, arches.S390X, nil, rng)
...
func TestMinimumSizes(t *testing.T) {
	...
	for idx, tc := range testCases {
		{ // without LVM
			mpt, err := disk.NewPartitionTable(&pt, tc.Blueprint, uint64(3*GiB), disk.RawPartitioningMode, arches.X86_64, nil, rng)
			assert.NoError(err)
		...
		{ // with LVM
			mpt, err := disk.NewPartitionTable(&pt, tc.Blueprint, uint64(3*GiB), disk.AutoLVMPartitioningMode, arches.X86_64, nil, rng)
			assert.NoError(err)
			for mnt, minSize := range tc.ExpectedMinSizes {
				path := disk.EntityPath(mpt, mnt)
				assert.NotNil(path, "[%d] mountpoint %q not found", idx, mnt)
...
func TestNewBootWithSizeLVMify(t *testing.T) {
	...
	mpt, err := disk.NewPartitionTable(&pt, custom, uint64(3*GiB), disk.AutoLVMPartitioningMode, arches.AARCH64, nil, rng)
	assert.NoError(err)
...
func TestMinimumSizesWithRequiredSizes(t *testing.T) {
	...
	for idx, tc := range testCases {
		{ // without LVM
			mpt, err := disk.NewPartitionTable(&pt, tc.Blueprint, uint64(3*GiB), disk.RawPartitioningMode, arches.AARCH64, map[string]uint64{"/": 1 * GiB, "/usr": 3 * GiB}, rng)
			assert.NoError(err)
			for mnt, minSize := range tc.ExpectedMinSizes {
				path := disk.EntityPath(mpt, mnt)
				assert.NotNil(path, "[%d] mountpoint %q not found", idx, mnt)
		...
		{ // with LVM
			mpt, err := disk.NewPartitionTable(&pt, tc.Blueprint, uint64(3*GiB), disk.AutoLVMPartitioningMode, arches.AARCH64, map[string]uint64{"/": 1 * GiB, "/usr": 3 * GiB}, rng)
			assert.NoError(err)
			for mnt, minSize := range tc.ExpectedMinSizes {
				path := disk.EntityPath(mpt, mnt)
				assert.NotNil(path, "[%d] mountpoint %q not found", idx, mnt)
