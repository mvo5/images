...
func TestNewCustomPartitionTable(t *testing.T) {
	...
	testCases := map[string]testCase{
		"null": {
			customizations: nil,
			options: &disk.CustomPartitionTableOptions{
				DefaultFSType:      disk.DefaultFSType{disk.FS_XFS},
		...
		"plain+": {
			...
			options: &disk.CustomPartitionTableOptions{
				DefaultFSType:      disk.DefaultFSType{disk.FS_EXT4},
				BootMode:           platform.BOOT_HYBRID,
		...
		"lvm": {
			...
			expected: &disk.PartitionTable{
				...
				UUID: "0194fdc2-fa2f-4cc0-81d3-ff12045b73c8",
				Size: 714*common.MebiByte + 116*common.MebiByte + 3*common.GibiByte + common.MebiByte, // start + size of last partition (VG) + footer
				Partitions: []disk.Partition{
					{
						Start:    1 * common.MebiByte, // header
						Size:     1 * common.MebiByte,
					...
					},
					{
						...
						Size:     3*common.GibiByte + 116*common.MebiByte + common.MebiByte - (disk.DefaultSectorSize + (128 * 128)), // the sum of the LVs (rounded to the next 4 MiB extent) grows by 1 grain size (1 MiB) minus the unaligned size of the header to fit the gpt footer
